...
### 5. Essential Spring Boot Annotations Reference

| Check | Annotation                 | Purpose                                                                           |
| :---- | :------------------------- | :-------------------------------------------------------------------------------- |
| ✅    | `@SpringBootApplication`   | Marks the main class; enables auto-configuration, component scan, and Spring Boot |
| ✅    | `@RestController`          | Combines `@Controller` + `@ResponseBody`; returns JSON/XML directly               |
| ✅    | `@RequestMapping`          | Base path for class or method; supports all HTTP methods                          |
...
| ✅    | `@RequestBody`             | Deserializes request body into Java object                                        |
|       | `@ResponseBody`            | Serializes return value to JSON/XML (implicit in `@RestController`)               |
...
|       | `@ResponseStatus`          | Sets HTTP status code for a response                                              |
|       | `@RestControllerAdvice`    | Global exception handler for REST APIs                                            |
|       | `@ExceptionHandler`        | Handles specific exceptions within a controller/advice                            |
|       | `@Valid` / `@Validated`    | Triggers JSR-303 validation on request bodies                                     |
...
|       | `@Autowired`               | Injects dependencies (prefer constructor injection)                               |
|       | `@RequiredArgsConstructor` | Lombok: generates constructor for `final` fields                                  |
|       | `@Bean`                    | Declares a bean within `@Configuration` class                                     |
